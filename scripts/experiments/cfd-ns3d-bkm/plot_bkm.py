#!/usr/bin/env python3
"""Plot 3D BKM diagnostic CSV → SVG for bigcompute.science."""
...
def svg_dual_axis(rows: list[dict[str, float]], *, width: int = 920, height: int = 420) -> str:
    ...
    parts: list[str] = [
        ...
        f'aria-label="3D NS max vorticity and BKM integral vs time">',
        '<rect width="100%" height="100%" fill="#0b1020"/>',
        f'<text x="{width/2:.1f}" y="24" fill="#e2e8f0" font-family="system-ui,sans-serif" '
        f'font-size="16" text-anchor="middle">3D NS — ||ω||∞ and BKM integral</text>',
    ]
    ...
    parts.append(
        f'<text x="{width-20}" y="{margin["t"]+16}" fill="#94a3b8" font-size="11" '
        f'font-family="system-ui,sans-serif" text-anchor="end">'
        f'final BKM={bkm[-1]:.3f}, max|ω|={max_w[-1]:.3f}</text>'
    )
    parts.append("</svg>")
    return "\n".join(parts)
