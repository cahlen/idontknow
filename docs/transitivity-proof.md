# Proof Attempt: Γ_{1,...,5} Generates SL₂(Z/pZ) for Every Prime p

> **Audit status (2026-04-23): incomplete / not citable as a proof.**
> This note contains a useful proof strategy, but it is not currently a
> rigorous all-prime proof. The generators
> \(g_a=\begin{psmallmatrix}a&1\\1&0\end{psmallmatrix}\) have determinant
> \(-1\), so the full generator set is not a subgroup of
> \(\mathrm{SL}_2(\mathbb F_p)\). A corrected proof must either work in
> \(\mathrm{GL}_2/\mathrm{PGL}_2\) or pass carefully to the even-word
> subgroup in \(\mathrm{SL}_2\). The Borel, Cartan-normalizer, and
> exceptional subgroup exclusions below also use computational orbit-size
> assertions in places where a purely algebraic proof is needed. The reliable
> current result is the finite computation to \(p=17{,}389\), documented on
> the website finding.

## Proposed Theorem (Incomplete)
...
## Proposed Corollary (Not Established)
